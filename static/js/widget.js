...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function add_instruction(event) {
        ...
        new_instruction = '<div class="add-instruction" id="' + instructionId + '"> <textarea type="text" name="instructions" cols="30" rows="3" class="instructions"></textarea> <button type="button" onClick="deleteInstruction(this)" class="delete_instruction" data-instruction-id="' + instructionId + '">Delete</button></div>';
    ...
    function add_new_instruction(event) {
        ...
        new_ingredient = '<div class="add-ingredient" id="' + ingredientId + '"> <textarea type="text" name="ingredients" cols="30" rows="3"  class="instructions"></textarea> <button type="button" onClick="deleteIngredient(this)" class="delete_ingredient" data-ingredient-id="' + ingredientId + '">Delete</button></div>';
        ingredientsCounter = ingredientsCounter + 1;
        let newingredient = document.createElement('div');
        newingredient.innerHTML = new_ingredient;
        document.getElementById('ingredients_list').appendChild(newingredient);
